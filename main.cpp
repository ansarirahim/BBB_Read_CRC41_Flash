...
int ReadFlashID_Cs(int fd, tByte Cs)
{
	...
	char readcommand[1] = {
		0x9f,
	}; // addresshigh, addressmid,addresslow ,};
	...
	for (unsigned int j = 0; j < sizeof(rx); j++)
...

{
	...
	////root/CLem/REad/ReadCrc
	fstream ofs;
	ofs.open("/root/CLem/REad/ReadCrc/crcflashcca", ios::out | ios::trunc);
	ofs << "Your content here";

	ofs.close();
	ofstream myfile;
	myfile.open("/root/CLem/REad/ReadCrc/crcflashcca");
	myfile << (int)yeed;
	myfile.close();
	////////////////////////////
	return 0;
}
static int Flash24ReadSingle_Crc_Cs(int fd, unsigned long int NoOfPages, tByte Css)
{ //int readnoofdevices) {
